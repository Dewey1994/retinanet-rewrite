...
class Conv2dSamePadding(nn.Module):
    ...
    def forward(self, x):
        ...
        h_step = math.ceil(h / self.stride[1])
        w_step = math.ceil(w / self.stride[0])
        # new h w thourgh the calculation of this formulation
        h_cover_len = self.stride[1] * (h_step - 1) + self.kernel_size[1]
        w_cover_len = self.stride[0] * (w_step - 1) + self.kernel_size[0]

        extra_h = h_cover_len - h
        extra_w = w_cover_len - w
...
class BIFPN(nn.Module):
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, attention=True):
        ...
        # Conv layers
        self.conv6_up = DepthWiseConvBlock(num_channels)
        self.conv5_up = DepthWiseConvBlock(num_channels)
        self.conv4_up = DepthWiseConvBlock(num_channels)
        self.conv3_up = DepthWiseConvBlock(num_channels)
        self.conv4_down = DepthWiseConvBlock(num_channels)
        self.conv5_down = DepthWiseConvBlock(num_channels)
        self.conv6_down = DepthWiseConvBlock(num_channels)
        self.conv7_down = DepthWiseConvBlock(num_channels)
        ...
        self.p4_downsample = MaxPool2dSamePadding(3, 2)  # 3=kernelsize 2=stride
        ...
        if self.first_time:
            self.p5_down_channel = nn.Sequential(
                DepthWiseConvBlock(conv_channels[2], num_channels, 1),
                nn.BatchNorm2d(num_channels, momentum=0.01, eps=1e-3),
            )
            self.p4_down_channel = nn.Sequential(
                DepthWiseConvBlock(conv_channels[1], num_channels, 1),
                nn.BatchNorm2d(num_channels, momentum=0.01, eps=1e-3),
            )
            self.p3_down_channel = nn.Sequential(
                DepthWiseConvBlock(conv_channels[0], num_channels, 1),
                nn.BatchNorm2d(num_channels, momentum=0.01, eps=1e-3),
            )
