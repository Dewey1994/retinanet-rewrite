...
class Detector(nn.Module):
    def __init__(self, pretrained=False):
        ...
        self.w_2_in = nn.Parameter(torch.Tensor(2, 6).fill_(1))  # 6代表两个input的结点的个数，2代表每个结点分配的权重
        self.w_3_in = nn.Parameter(torch.Tensor(3, 2).fill_(1))  # 2代表两个input的结点的个数，3代表每个结点分配的权重
        self.eps = 0.0001
        self.w_relu = nn.ReLU()
        # ---------------------------
        ...
        self.bi1_prj2_5 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi1_prj2_4 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi1_prj2_3 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi1_prj3_5 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi1_prj3_4 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi1_prj3_3 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        # bifpn ======================================================
        self.bi2_prj1_5 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj1_4 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj1_3 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj2_5 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj2_4 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj2_3 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj3_5 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj3_4 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bi2_prj3_3 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        # new bifpn ==================================================
    ...
    def downsample(self, input):
        return F.interpolate(input, size=(int(input.shape[2]/2)+1, int(input.shape[3]/2)+1),
    ...
    def forward(self, x, loc, label_class=None, label_box=None):
        ...
        E4 = self.bi1_prj2_4((D4 +self.upsample(D5)))


        F3 = self.bi1_prj3_3(( D3 + self.upsample(E4)))
        F4 = self.bi1_prj3_4((self.downsample(F3) + E4 + D4))
        F5 = self.bi1_prj3_5((D5 + self.downsample(F4)))



        G4 = self.bi2_prj2_4((F4 + self.upsample(F5)))


        H3 = self.bi2_prj3_3((F3 + self.upsample(G4)))
        H4 = self.bi2_prj3_4((self.downsample(H3) + G4 + F4))
        H5 = self.bi2_prj3_5((F5 + self.downsample(H4)))
