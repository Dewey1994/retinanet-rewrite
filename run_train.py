...
def aug_func(img, boxes):
    ...
    if random.random() < 0.5:
        img, boxes = augment.random_crop_resize(img, boxes, size=512, 
                        ...
                        attempt_max=10)
    if random.random() < 0.5:
        img, boxes = augment.random_mirror(img, boxes)
    return img, boxes
if cfg['augmentation']:
...
#     lr = lr_base
#     if step < WARM_UP_ITERS:
#         multilr=0.5*(1-cos(pi*(step/float(WARM_UP_ITERS))))
#     else:
#         multilr=0.5*(1-cos(pi*((step-WARM_UP_ITERS)/float(max_step-WARM_UP_ITERS))))
#     return float(lr*multilr)
