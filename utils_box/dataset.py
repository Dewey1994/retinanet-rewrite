...
class Dataset_CSV(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            # standard procedure #$##########&6712641782641782461827461278
            if random.random() < 0.5:
            ...
            #459834958349583049583049583904
        else:
            img, boxes, loc, scale = center_fix(img, boxes, self.size)
...
def random_resize_fix(img, boxes, size, img_scale_min):
    ...
    img = img.resize((ow,oh), Image.BILINEAR) # resize -> ow oh
    if boxes.shape[0] != 0:
    ...
    img = img.crop((-ofst_w, -ofst_h, size-ofst_w, size-ofst_h))  # 保持img的size和crop之前一样
    if boxes.shape[0] != 0:
        boxes += torch.FloatTensor([ofst_h, ofst_w, ofst_h, ofst_w])
    loc = torch.FloatTensor([ofst_h, ofst_w, ofst_h+oh, ofst_w+ow])
...
if __name__ == '__main__':
    ...
    augmentation = None
    batch_size = 8
    csv_root  = '/home/beihangvr/data/VOCdevkit/VOC2007/JPEGImages'
    ...
    for i, (imgs, boxes, labels, locs, scales) in enumerate(dataloader):
        print(imgs.shape)
        print(boxes.shape)
        print(labels.shape)
        print(locs.shape)
